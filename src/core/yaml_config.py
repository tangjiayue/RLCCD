...
class YAMLConfig(BaseConfig):
    ...
    def evaluator(self):
        if self._evaluator is None and "evaluator" in self.yaml_cfg:
            if self.yaml_cfg["evaluator"]["type"] == "CocoEvaluator":
                ...
                # # 调试：过滤前的统计
                # original_count = len(base_ds.dataset['annotations'])
                # categories_before = set(ann['category_id'] for ann in base_ds.dataset['annotations'])
                # print(f"\n🔍 过滤前:")
                # print(f"  总标注数: {original_count}")
                # print(f"  类别: {sorted(categories_before)}")
                # print(f"  类别0数量: {sum(1 for ann in base_ds.dataset['annotations'] if ann['category_id'] == 0)}")

                # # 过滤
                # base_ds.dataset['annotations'] = [
                #     ann for ann in base_ds.dataset['annotations'] 
                #     if ann['category_id'] != 0
                # ]
                
                # # 调试：过滤后的统计
                # filtered_count = len(base_ds.dataset['annotations'])
                # categories_after = set(ann['category_id'] for ann in base_ds.dataset['annotations'])
                # print(f"\n✅ 过滤后:")
                # print(f"  总标注数: {filtered_count}")
                # print(f"  类别: {sorted(categories_after)}")
                # print(f"  减少标注数: {original_count - filtered_count}")
    ...
    def build_dataloader(self, name: str):
        ...
        if "accum_steps" in global_cfg[name]:
            # pop unexpected key for dataloader init
            _ = global_cfg[name].pop("accum_steps")
