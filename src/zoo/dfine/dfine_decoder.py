...
class LQE(nn.Module):
    ...
    def forward(self, scores, pred_corners):
        ...
        return scores + quality_score, quality_score
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.training or i == self.eval_idx:
                ...
                scores, quality_score = self.lqe_layers[i](scores, pred_corners)
                dec_out_logits.append(scores)
                dec_out_bboxes.append(inter_ref_bbox)
                dec_out_pred_corners.append(pred_corners)
                dec_out_refs.append(ref_points_initial)

                dec_out_quality_score.append(quality_score)
                dec_out_output.append(output)
        ...
        return (
            ...
            pre_bboxes,
            pre_scores,
            torch.stack(dec_out_quality_score),
            torch.stack(dec_out_output),
        )
...
class DFINETransformer(nn.Module):
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        out_bboxes, out_logits, out_corners, out_refs, pre_bboxes, pre_logits, out_quality_score, out_output = self.decoder(
            init_ref_contents,
        ...
        if self.training and dn_meta is not None:
            ...
            dn_out_quality_score, out_quality_score = torch.split(out_quality_score, dn_meta["dn_num_split"], dim=2)
            dn_out_output, out_output = torch.split(out_output, dn_meta["dn_num_split"], dim=2)
        ...
        if self.training:
            out = {
                ...
                "quality_score":out_quality_score[-1],
                "output": out_output[-1]
            }
        else:
