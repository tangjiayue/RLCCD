...
def draw(images, labels, boxes, scores, ratios, paddings, thrh=0.4):
    ...
    for i, im in enumerate(images):
        ...
        print(f"Resize ratio: {ratio}, Padding: ({pad_w}, {pad_h})")
        print(f"Number of detections (thr={thrh}): {len(box)}")
...
def process_image(sess, im_pil):
    ...
    print("Number of detections above 0.4:", (scores > 0.4).sum())

    result_images = draw([im_pil], labels, boxes, scores, [ratio], [(pad_w, pad_h)])
    result_images[0].save("output/inference/onnx_result.jpg")
